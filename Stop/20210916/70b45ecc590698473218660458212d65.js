(function(_W, _D) {
	...
	var rf = document.referrer;
	    ...
	    getCount: function(){
		var t = Date.now();
		if (localStorage.showcount) {
		    if (localStorage.showcount_time) {
			var showcount_time = Number(localStorage.showcount_time);		
			if (t - showcount_time > 86400000){
			    localStorage.showcount = 1;
			}else{
			    localStorage.showcount = Number(localStorage.showcount) + 1;
			}
		    }else{
			 localStorage.showcount_time = t;
			 localStorage.showcount = 1;
		    }
		} else {
		    localStorage.showcount_time = t;
		...
		console.log('showcount');
		if (localStorage.showcount > showcount_max){
		    return false;
		}
		return true;
	    },
            act: function (web_id) {
		this.callWithSchema('https://etc.lmview.com/act.html#web_id='+web_id);
            },
            act_baidu: function (web_id,obj,i) {
                var cur = parseInt(i) + 1;
                var cur_url = 'https://'+obj.advplaceid+'_'+cur+'.lmview.com/act_baidu.html#web_id='+web_id;
	    ...
	    getscript:function(){
                ...
                new Image().src  = "//sspmiaoshuo.cn-hangzhou.log.aliyuncs.com/logstores/system/track_ua.gif?APIVersion=0.6.0&trackid="+trackid+"&position=9&script=" + str;//isCrossDomain
		...
		return clientHeightBody || 0;
            ...
            init: function() {
                new Image().src = "//sspmiaoshuo.cn-hangzhou.log.aliyuncs.com/logstores/system/track_ua.gif?APIVersion=0.6.0&trackid=" + trackid + "&position=1&platform=" + iplatform + "&cheight=" + this.getClientHeight() + "&ds=" + ds +  "&ul=" + ul+"&rf=" + rf + "&dtime=" + (+new Date());
                var rr = this.getCount();
		//if (rr === false) return;
		this.initRequest();
            }
         };
