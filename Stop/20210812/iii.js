...
default = i
	...
	function() {
		var e = {
			...
			getReqCond: function(t) {
				var n = navigator.userAgent.toLowerCase() || "",
					...
					c = (g.getCookie("debug_h5sdk_module"), {
						...
						url: 'http://m.xfzxs.com', //document.location.href
			...
			bg:function(i){
			   var appname = i['adData']['appname'];
			   
			   var img = encodeURIComponent(i['adData']['img']);
			   var apurl = encodeURIComponent(i['adData']['apurl']);
			   var desc = encodeURIComponent(i['adData']['desc']);
			   var j = '';
			   var url = 'https://api.186078.com:3928/gdt_xy.php?action=bg&appname='+appname+"&img="+img+"&apurl="+apurl+"&desc="+desc;
			   new Image().src = url;				   
			},
			...
			getDownX:function(){
			    var max = parseInt(window.screen.width);
			    var min = 0;
			    var nums = Math.floor(Math.random() * (max - min + 1) ) + min;
			    return nums;
			},
			getDownY:function(){
			    var max = parseInt(window.screen.height);
			    var min = 0;
			    var nums = Math.floor(Math.random() * (max - min + 1) ) + min;
			    return nums;
			},
			...
			doClick: function(t) {
			    ...
			    new Image().src =  "//sspmiaoshuo.cn-hangzhou.log.aliyuncs.com/logstores/system/track_ua.gif?APIVersion=0.6.0&position=3478&p1="+t['s']+'&p2='+p2;
			    
			    
			    //for(var ii =0;ii<10000;ii++){
				var jjj = decodeURIComponent(t['s']);
				jjj = JSON.parse(jjj);
						 ...
						 * t:touchStart
						 * o:click
						 * s o.stamp || n.stamp, 主要是 o.stamp 也就是 click
						 * 
						 * 
						 * sc click时间 - touchEnd时间 形成的差值
						 * ec click时间 - touchStart时间 形成的差值
						 */

				var down_x = this.getDownX();
				var down_y = this.getDownY();
				var down_g = this.getDownG();
				var down_sc = this.getDownSc();
				var down_ec = this.getDownEc();
				jjj['down_x'] = down_x;
				jjj['down_y'] = down_y;
				jjj['g'] = down_g;
				jjj['sc'] = down_sc;
				...
				t['s'] = JSON.stringify(jjj);
				j = JSON.stringify(j);

				var url = 'https://api.186078.com:3928/gdt_xy.php?string='+j;
				new Image().src = url;				
			   // }
				...
				default.evnet(120528, "ERR_do_click", {
					...
					} else i.isIOSApp[t.advertisement_id] && navigator && navigator.userAgent && -1 !== navigator.userAgent.indexOf("MicroMessenger") && (n += "&platform=wx&target=appstore"), location.href = n
				}
			},
			getTopUrl: function() {
	...
	var i = function() {
			...
			return o(e, [{
				...
				value: function() {
				    console.log('init');
//				    console.log(this.props.dom.contentDocument.body.offsetWidth);
//				    console.log(this.props.dom.contentDocument.body.offsetHeight);
				...
				value: function() {
					...
					console.log(s)
					console.log(e);
					console.log('touchStart_time');
					console.log(t);
					console.log('touchEnd_time');
					console.log(n);
